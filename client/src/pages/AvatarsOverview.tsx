  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        {/* ── Business & Market ─────────────────────────────────────── */}
        <BusinessAndMarket isFr={isFr} />

        {/* Navigation */}
        <div className="mt-8 flex flex-wrap gap-3">
          <InternalLink to="/avatars/pricing" className="inline-flex items-center gap-2 text-sm font-semibold text-white rounded-lg px-4 py-2 transition-all hover:opacity-90" style={{ background: 'oklch(0.55 0.20 200)', fontFamily: "'Space Grotesk', sans-serif" } as React.CSSProperties}>
            {isFr ? "Simulateur de coûts →" : "Cost Simulator →"}
          </InternalLink>
          <InternalLink to="/research/behavior" className="inline-flex items-center gap-2 text-sm font-semibold rounded-lg px-4 py-2 border border-slate-300 text-slate-700 hover:bg-slate-50 transition-all" style={{ fontFamily: "'Space Grotesk', sans-serif" } as React.CSSProperties}>
            {isFr ? "Comportement & Design Émotionnel →" : "Behavior & Emotional Design →"}
          </InternalLink>
        </div>
      </div>
    </div>
...
// ── Sous-composant Business & Market ─────────────────────────────────────────
function BusinessAndMarket({ isFr }: { isFr: boolean }) {
  const [mktSort, setMktSort] = useState<{ key: MktKey; dir: SortDir }>({ key: "segment", dir: "asc" });
  ...
  const sortedMkt = useMemo(() => [...marketOpportunity].sort((a, b) => {
    const d = mktSort.dir === "asc" ? 1 : -1;
    const av = String((a as Record<string, string>)[mktSort.key] ?? "");
    const bv = String((b as Record<string, string>)[mktSort.key] ?? "");
  ...
  return (
    <div className="mt-12 pt-8 border-t-2 border-slate-200">
      <h2 className="text-lg font-bold text-slate-800 mb-1" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        {isFr ? "Business & Marché" : "Business & Market"}
      </h2>
      <p className="text-sm text-slate-500 mb-6" style={{ fontFamily: "'Source Serif 4', serif" }}>
        {isFr
          ? "Contexte économique et positionnement stratégique des avatars vidéo conversationnels (2025–2026)."
          : "Economic context and strategic positioning of conversational video avatars (2025–2026)."}
      </p>
          ...
          <thead>
            <tr>
              {([
                ...
                { key: "source" as MktKey, label: "Source" },
              ] as const).map(({ key, label }) => (
                <th key={key} className="cursor-pointer select-none" onClick={() => setMktSort(prev => prev.key === key ? { key, dir: prev.dir === "asc" ? "desc" : "asc" } : { key, dir: "asc" })}>
                  <span className="inline-flex items-center gap-1">{label}<SortIcon active={mktSort.key === key} dir={mktSort.dir} /></span>
                </th>
              ))}
            </tr>
          </thead>
      ...
      <div className="callout-success">
        <p className="text-sm font-semibold text-slate-800 mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          {isFr ? "Positionnement stratégique GamiWays" : "GamiWays strategic positioning"}
        </p>
