                ...
                {[
                  {
                    ...
                    gap: isFr ? "Pas de solution production-grade pour sessions 1h+ sans explosion tokens" : "No production-grade solution for 1h+ sessions without token explosion",
                    sota: isFr ? "Mem0 (-90% tokens, +26% précision) — mais pas validé pour avatars multi-sessions" : "Mem0 (-90% tokens, +26% accuracy) — but not validated for multi-session avatars",
                    opportunity: isFr ? "Architecture 3 couches + distillation SLM spécifique à l'avatar" : "3-layer architecture + avatar-specific SLM distillation",
                    urgency: isFr ? "Critique" : "Critical",
                  },
                  {
                    domain: isFr ? "Fidélité comportementale avatar" : "Avatar behavioral fidelity",
                    gap: isFr ? "Avatars 'talking heads' sans langage corporel — uncanny valley de familiarité" : "'Talking heads' avatars without body language — familiarity uncanny valley",
                    sota: isFr ? "VASA-1 (Microsoft) : 40 FPS, expressions nuancées — non commercialisé" : "VASA-1 (Microsoft): 40 FPS, nuanced expressions — not commercialized",
                    opportunity: isFr ? "Extraction comportementale depuis archives + génération corps cohérent" : "Behavioral extraction from archives + coherent body generation",
                    urgency: isFr ? "Critique" : "Critical",
                  },
                  {
                    domain: isFr ? "TTS prosodique personnalisé" : "Personalized prosodic TTS",
                    gap: isFr ? "Cloner l'empreinte prosodique individuelle (rythme, emphase, pauses) reste difficile" : "Cloning individual prosodic fingerprint (rhythm, emphasis, pauses) remains difficult",
                    sota: isFr ? "FishAudio S1 : timbre + style depuis ~10s — mais prosodie profonde non capturée" : "FishAudio S1: timbre + style from ~10s — but deep prosody not captured",
                    opportunity: isFr ? "Modèles prosodiques individuels depuis archives vidéo existantes" : "Individual prosodic models from existing video archives",
                    urgency: isFr ? "Élevée" : "High",
                  },
                  {
                    domain: isFr ? "Latence avatar end-to-end" : "End-to-end avatar latency",
                    gap: isFr ? "6–12s actuels vs <2s requis — goulot : génération vidéo avatar" : "Current 6–12s vs <2s required — bottleneck: avatar video generation",
                    sota: isFr ? "Beyond Presence <100ms, NVIDIA ACE <100ms — mais infrastructure propriétaire" : "Beyond Presence <100ms, NVIDIA ACE <100ms — but proprietary infrastructure",
                    opportunity: isFr ? "Distillation + cache intelligent + dégradation gracieuse sur GPU souverain" : "Distillation + intelligent cache + graceful degradation on sovereign GPU",
                    urgency: isFr ? "Critique" : "Critical",
                  },
                  {
                    domain: isFr ? "Orchestration déterministe-organique" : "Deterministic-organic orchestration",
                    gap: isFr ? "Équilibre contraintes narratives / liberté IA conversationnelle non résolu" : "Balance between narrative constraints / conversational AI freedom unresolved",
                    sota: isFr ? "Flowise + custom : possible mais fragile et technique" : "Flowise + custom: possible but fragile and technical",
                    opportunity: isFr ? "Éditeur de nœuds avec degrés de liberté configurables (0–100%)" : "Node editor with configurable degrees of freedom (0–100%)",
                    urgency: isFr ? "Élevée" : "High",
                  },
                  {
                    domain: isFr ? "Synchronisation multi-flux" : "Multi-stream synchronization",
                    gap: isFr ? "<100ms désynchronisation entre 5 streams parallèles en conditions réelles" : "<100ms desynchronization between 5 parallel streams in real conditions",
                    sota: isFr ? "WebRTC + HLS + WebSocket — solutions partielles, pas de framework unifié" : "WebRTC + HLS + WebSocket — partial solutions, no unified framework",
                    opportunity: isFr ? "Protocole de synchronisation adaptatif basé sur 14 ans d'expertise Memoways" : "Adaptive synchronization protocol based on 14 years of Memoways expertise",
                    urgency: isFr ? "Moyenne" : "Medium",
                  },
                ].map((g) => {
                  const uc = g.urgency === "Critical" || g.urgency === "Critique"
                    ? { bg: "#fee2e2", text: "#dc2626" }
                    : g.urgency === "High" || g.urgency === "Élevée"
