  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        latencyBest: 120,
        latencyTypical: 200,
        costNote: "$5/1M chars ($0.000005/char) — ~0.5¢/min",
        costPerMin: 0.005,
        sovereign: true,
...
export const recommendedStacks: RecommendedStack[] = [
  ...
  },
  {
    ...
    rationale: "Key advantage: Inworld STT + LLM Router + TTS share the same internal infrastructure — no inter-component network serialization. Estimated 30–50ms latency gain vs multi-provider cascade. WebRTC (10ms) + Inworld STT (80ms) + Mem0 (20ms) + GPT-4o streaming (350ms) + Inworld TTS Mini (120ms) + WebRTC (30ms) = ~490ms best-case. On-premise option enables Swiss sovereign deployment. Voice cloning + viseme timestamps included.",
    rationaleFr: "Avantage clé : Inworld STT + LLM Router + TTS partagent la même infrastructure interne — pas de sérialisation réseau inter-composants. Gain estimé de 30–50ms vs cascade multi-fournisseurs. WebRTC (10ms) + Inworld STT (80ms) + Mem0 (20ms) + GPT-4o streaming (350ms) + Inworld TTS Mini (120ms) + WebRTC (30ms) = ~490ms meilleur cas. Option on-premise pour déploiement souverain suisse. Clonage vocal + timestamps visèmes inclus.",
  },
  {
    id: "hybrid-stack",
    label: "Hybrid Stack",
