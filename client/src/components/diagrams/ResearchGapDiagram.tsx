...
export default function ResearchGapDiagram() {
  ...
  function plotY(difficulty: number) { return PAD + 20 + ((5 - difficulty) / 4) * PLOT_H; }
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        <text x={PAD + PLOT_W * 0.75} y={PAD + 36} textAnchor="middle" fontSize="9" fill="#dc2626" fontFamily="'JetBrains Mono', monospace" opacity="0.7">
          {isFr ? "CRITIQUE" : "CRITICAL"}
        </text>

        <rect x={PAD} y={PAD + 20} width={PLOT_W / 2} height={PLOT_H / 2} fill="#fffbeb" opacity="0.5" />
        <text x={PAD + PLOT_W * 0.25} y={PAD + 36} textAnchor="middle" fontSize="9" fill="#d97706" fontFamily="'JetBrains Mono', monospace" opacity="0.7">
          {isFr ? "COMPLEXE" : "COMPLEX"}
        </text>

        <rect x={PAD + PLOT_W / 2} y={PAD + 20 + PLOT_H / 2} width={PLOT_W / 2} height={PLOT_H / 2} fill="#f0fdf4" opacity="0.5" />
        <text x={PAD + PLOT_W * 0.75} y={PAD + 20 + PLOT_H * 0.75 + 6} textAnchor="middle" fontSize="9" fill="#16a34a" fontFamily="'JetBrains Mono', monospace" opacity="0.7">
          {isFr ? "PRIORITAIRE" : "PRIORITY"}
        </text>

        <rect x={PAD} y={PAD + 20 + PLOT_H / 2} width={PLOT_W / 2} height={PLOT_H / 2} fill="#f8fafc" opacity="0.5" />
        <text x={PAD + PLOT_W * 0.25} y={PAD + 20 + PLOT_H * 0.75 + 6} textAnchor="middle" fontSize="9" fill="#94a3b8" fontFamily="'JetBrains Mono', monospace" opacity="0.7">
        ...
        <text x={PAD - 30} y={PAD + 20 + PLOT_H / 2} textAnchor="middle" fontSize="10" fill="#374151" fontWeight="500"
        ...
        {gaps.map((gap) => {
          ...
          return (
            <g key={gap.abbrev}>
              ...
              <text x={x} y={y - 2} textAnchor="middle" fontSize="10" fontWeight="700" fill={gap.color} fontFamily="'JetBrains Mono', monospace">{gap.abbrev}</text>
              <text x={x} y={y + 10} textAnchor="middle" fontSize="7" fill={gap.color} fontFamily="'JetBrains Mono', monospace">{gap.urgency}/{gap.difficulty}</text>
            </g>
          );
        })}
        ...
        <g transform={`translate(${PAD}, ${H - 28})`}>
          {gaps.map((gap, i) => (
            <g key={gap.abbrev} transform={`translate(${i * 80}, 0)`}>
              ...
              <text x="8" y="12" textAnchor="middle" fontSize="7" fontWeight="700" fill={gap.color} fontFamily="'JetBrains Mono', monospace">{gap.abbrev}</text>
            </g>
          ))}
        </g>
